...
def do_symmlq(Aperm, KKT, R, perm, perminv, block_L=None, schur_diag=None, KKT_diag=None):
    ...
    KKT_diag = torch.ones_like(KKT_diag)
...
def QPFunction(pde, n_iv, n_step=10, gamma=1, alpha=1, double_ret=True):
    class QPFunctionFn(Function):
        ...
        #def forward(ctx, coeffs, rhs, iv_rhs):
            ...
            #u = l
            def get_diag(us=1e3, ds=1e-5, device='cpu'):
                _P_diag = torch.ones(num_eps, dtype=rhs.dtype, device=device)*us
                _P_zeros = torch.zeros(num_var, dtype=rhs.dtype, device=device) +ds
            ...
            def make_kkt(us=1e3, ds=1e-5):
                ...
                P_diag = get_diag(us=us, ds=ds, device='cpu')
            ...
            KKT_top = get_diag(us=1e2, ds=0, device=rhs.device)
            #KKT_top2 = get_diag(us=1e2, ds=0, device=rhs.device)
            ...
            sol, info, block_L = do_minres(Aperm, KKT, R,perm=pde.row_perm, perminv=pde.row_perm_inv,
                                  schur_diag=None, KKT_diag=KKT_top, num_qvar=num_var)
            print('minres ', info)
