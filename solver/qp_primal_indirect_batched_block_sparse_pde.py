...
import ipdb
#from solver.sparse_qr_solver import get_permutation
#from solver.sparse_qr_solver import BatchedSparseQR
...
def solve_kkt_indirect_cg(A, AAt, g, h, gamma):
    """
        Solve min x'Gx + d'x
            ...
            G := gamma*I
    """
    #print(A.shape, AAt.shape, g.shape, h.shape)
    #ipdb.set_trace() 
    #At = A.transpose(1,2)
    ...
    #rhs1 = A@g - gamma*h
    #rhs1 = torch.bmm(A,g) - gamma*h
    #rhs1 = bmm_fix(A,g) - gamma*h
    ...
    #y,info = CPSLA.cg(AAt, rhs, maxiter=100)
    #y,info = spla.cg(AAt, rhs, maxiter=100)
    #y,info = CPSLr.gmres(AAt, rhs, maxiter=50)
    #assert(info==0)
    assert(info>=0)
    #res = rhs - AAt@y
    #res = rhs - torch.bmm(AAt,y.unsqueeze(-1)).squeeze()
    #res = rhs1 - block_mv(AAt,y)
    #res = torch.bmm(AAt,y.unsqueeze(-1))
    #print('done ', res.mean().item() )
    
    #y = y.squeeze(-1)
    #y = torch.tensor(y).type_as(rhs1).unsqueeze(0)
    #p = At@y - g
    #p = torch.bmm(At,y.unsqueeze(-1)) - g
    #p = bmm_fix(At,y.unsqueeze(-1)) - g
    p = block_mv(At,y) - g
    p = p/gamma
