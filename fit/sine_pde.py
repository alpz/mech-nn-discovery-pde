...
class Method(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        loss = (u0.reshape(-1)-y.reshape(-1)).pow(2).sum()
...
class Sine(nn.Module):
    def __init__(self, bs=1, device=None):
        ...
        if self.n_iv > 0:
            ...
            iv_rhs = torch.zeros(1,2*self.coord_dims[1] + 2*self.coord_dims[0], dtype=dtype)
        ...
        self.steps1 = torch.logit(self.step_size*torch.ones(1,1,self.pde.step_grid_shape[1][1]))
    ...
    def forward(self, check=False):
        ...
        steps0 = self.steps0.repeat(1,1,self.pde.step_grid_shape[0][1]).type_as(coeffs)
        steps1 = self.steps1.repeat(1,self.pde.step_grid_shape[1][0],1).type_as(coeffs)
        #steps0 = self.steps0
        #steps1 = self.steps1
