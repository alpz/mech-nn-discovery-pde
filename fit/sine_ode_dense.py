...
class SineDataset(Dataset):
    ...
    def __getitem__(self, idx):
        y = self.y0 #+ 0.5*torch.randn_like(self.y)
        return y
...
class Sine(nn.Module):
    def __init__(self, bs=1, device=None):
        ...
        self.iv_list = [lambda nx: (0,0, [0],[0]), 
                        lambda nx:(0,0, [nx-1], [nx-1]), 
                        ]
    ...
    def forward(self, init_list, check=False):
        ...
        #iv_rhs = torch.tensor(init_list, dim=0)
        iv_rhs = torch.tensor(init_list, device=coeffs.device)
