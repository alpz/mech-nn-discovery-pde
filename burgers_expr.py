...
class Model(nn.Module):
    def __init__(self, bs, solver_dim, steps, device=None, **kwargs):
        ...
        self.data_conv2d = nn.Sequential(
            nn.Conv2d(3, 64, kernel_size=5, padding=2, stride=1, padding_mode=pm),
            #nn.ELU(),
            nn.ReLU(),
    ...
    def forward(self, u, t, x):
        ...
        up_rhs = self.data_conv2d(cin)
        up_coeffs = self.up_coeffs.repeat(self.bs,self.pde.grid_size,1)
        #up_rhs = up

        steps0 = self.steps0.type_as(up_coeffs).expand(-1, self.coord_dims[0]-1)
        steps1 = self.steps1.type_as(up_coeffs).expand(-1, self.coord_dims[1]-1)
        ...
        u0,u,eps = self.pde(up_coeffs, up_rhs, iv_rhs, steps_list)

        up = u0.reshape(bs, *self.coord_dims)
        up_t = u[...,1].reshape(bs, *self.coord_dims)
        up_x = u[...,2].reshape(bs, *self.coord_dims)
        up_tt = u[...,3].reshape(bs, *self.coord_dims)
        up_xx = u[...,4].reshape(bs, *self.coord_dims)


        p = torch.stack([0*torch.ones_like(up), up, up**2, 0*up**3], dim=-1)
        q = torch.stack([torch.ones_like(up), up, up**2, up**3], dim=-1)

        #p = torch.stack([0*torch.ones_like(up), up], dim=-1)
