...
class PorousDataset(Dataset):
    def __init__(self, solver_dim=(32,32)):
        ...
        print('steps', self.t_step, self.x_step)
        ...
        self.data_ux = torch.tensor(data_ux, dtype=dtype)#.permute(1,0) 
        print('data shapes ', self.data.shape, self.data_ux.shape)
    ...
    def __getitem__(self, idx):
        ...
        data_ux = self.data_ux[t_idx:t_idx+t_step, x_idx:x_idx+x_step]

        return data, data_ux

#%%

ds = PorousDataset(solver_dim=solver_dim)#.generate()
...
class Model(nn.Module):
    ...
    def forward(self, u):
        ...
        u0, u0x, eps = self.solve_chunks(u_patched, up_patched, up2_patched, params)
        ...
        return u0, u0x, up,up2, eps, params
...
def optimize(nepoch=5000):
    ...
    for epoch in range(nepoch):
        ...
        #for i, batch_in in enumerate((train_loader)):
            optimizer.zero_grad()
            #batch_u,batch_u_x, t,x = batch_in[0], batch_in[1], batch_in[2], batch_in[3]
            ...
            u0, u0x, var, var2, eps, params = model(batch_u)
            ...
            #bs = batch_in.shape[0]
            #batch_in = batch_in.reshape(bs, -1)#[:, :t_end, :x_end]
            #var = var.reshape(bs, -1)#[:, :t_end, :x_end]
            #var2 = var2.reshape(bs, -1)#[:, :t_end, :x_end]
        ...
        _u_loss = torch.cat(u_losses,dim=0).mean()
        _v_loss = torch.cat(var_losses,dim=0).mean()
        _vx_loss = torch.cat(var2_losses,dim=0).mean()
