...
class BrusselatorDataset(Dataset):
    def __init__(self, solver_dim=(32,32)):
        ...
        u_data=np.load(os.path.join(PDEConfig.euler_dir, 'euler_u.npy'))
        v_data=np.load(os.path.join(PDEConfig.euler_dir, 'euler_v.npy'))
        p_data=np.load(os.path.join(PDEConfig.euler_dir, 'euler_gradp_x.npy'))
        ts_data=np.load(os.path.join(PDEConfig.euler_dir, 'euler_t.npy'))
        #downsample time
        #u_data = u_data[::downsample]
        #v_data = v_data[::downsample]
        L.info(f'Euler {u_data.shape}')
        #L.info(f'downsample {downsample}')

        self.t_step = 0.005
        self.x_step = 0.0078125
        self.y_step = 0.0078125


        if noise:
            percent = 20
            frac = percent/100
            L.info(f'adding {percent}% noise')
            ...
            u_data = u_data + np.random.normal(0, rmse *frac, u_data.shape) 
            v_data = v_data + np.random.normal(0, rmse *frac, v_data.shape) 
            #p_data = p_data + np.random.normal(0, rmse *frac, uv_data.shape) 

        u_data = torch.tensor(u_data, dtype=dtype)#.permute(1,0,2,3) 
        v_data = torch.tensor(v_data, dtype=dtype)#.permute(1,0,2,3) 
        p_data = torch.tensor(p_data, dtype=dtype)#.permute(1,0,2,3) 
        ts_data = torch.tensor(ts_data, dtype=dtype)#.permute(1,0,2,3) 
        #convert to step
    ...
    def __getitem__(self, idx):
        ...
        v_data = self.v_data[t_idx:t_idx+t_step, 
                             x_idx:x_idx+x_step,
                             y_idx:y_idx+y_step]
        p_data = self.p_data[t_idx:t_idx+t_step, 
                             x_idx:x_idx+x_step,
                             y_idx:y_idx+y_step]
        ...
        return u_data, v_data, p_data, t
        #return v_data, u_data, t, x, y
...
class Model(nn.Module):
    ...
    def get_steps(self, u, t):
        ...
        steps0 = t.type_as(u)[:, :-1]#.expand(self.bs, -1)
        ...
        #ipdb.set_trace()
        #print(steps0.shape, steps1.shape, steps2.shape)
    ...
    def solve(self, u, v, up, vp, p, params_list, steps_list):
        bs = u.shape[0]
        ...
        p = p.reshape(bs, self.pde.grid_size)
        ...
        coeffs_u[..., 2] =  params_list[3][0]*up[:,0]
        coeffs_u[..., 3] =  params_list[3][1]*vp[:,0]
    ...
    def forward(self, u, v, p, t):
        ...
        p_in = p.unsqueeze(1) #torch.stack([v,t, x,y], dim=1) 
        ...
        u0, v0, rhs_loss = self.solve(u,v, up, vp,pp, params, steps_list)

        return u0, v0,up,vp,pp, params
...
def optimize(nepoch=5000):
    ...
    for epoch in range(nepoch):
        ...
        #for i, batch_in in enumerate((train_loader)):
            optimizer.zero_grad()
            batch_u, batch_v, batch_p = batch_in[0], batch_in[1], batch_in[2]
            t = batch_in[3]
            batch_u = batch_u.double().to(device)
            batch_v = batch_v.double().to(device)
            batch_p = batch_p.double().to(device)
            ...
            u, v, var_u, var_v,pp, params = model(batch_u, batch_v, batch_p, t)
            ...
            #var_u_loss = (var_u- batch_u).abs().mean(dim=-1)
            #var_u_loss = (var_u- batch_u).pow(2).mean(dim=-1)
            var_u_loss = (var_u- u.unsqueeze(1)).abs().mean(dim=-1)
            var_v_loss = (var_v- batch_v.unsqueeze(1)).abs().mean(dim=-1)
            p_loss = (pp- batch_p).abs().mean(dim=-1)
            ...
            loss = u_loss.mean() +  var_u_loss.mean() + var_v_loss.mean()  + p_loss.mean()
            ...
            p_losses.append(p_loss.mean().detach().item())
            ...
            del loss,u,u_loss,v,v_loss,var_u,var_v,var_u_loss,var_v_loss,p_loss, params#, t_params_loss
        ...
        _p_loss = torch.tensor(p_losses).mean().item()
