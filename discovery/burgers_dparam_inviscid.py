...
class BurgersDataset(Dataset):
    def __init__(self, solver_dim=(32,32)):
        ...
        data=np.load(os.path.join(PDEConfig.burgers_dir, 'burgers_inviscid.npy'))
...
class Model(nn.Module):
    def __init__(self, bs, solver_dim, steps, device=None, **kwargs):
        ...
        self.rnet1_1d = N.ResNet1D(out_channels=1, in_channels=1)
        self.rnet2_1d = N.ResNet1D(out_channels=1, in_channels=1)
...
def optimize(nepoch=5000):
    ...
    for epoch in range(nepoch):
        ...
        #for i, batch_in in enumerate((train_loader)):
            ...
            loss = x_loss.mean() + var_loss.mean()  +  0.05*param_loss.mean()
            #loss = x_loss.mean()  +  0.05*param_loss.mean()
